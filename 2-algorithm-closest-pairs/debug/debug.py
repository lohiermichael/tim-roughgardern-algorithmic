...
def count_percentage_err(
    algorithm=divide_to_conquer, nb_tests: int = 1000, l_length: int = 10
):
    bad = 0
...
if __name__ == "__main__":
    print(
        f"There is {count_percentage_err(algorithm=divide_to_conquer)} % of error on {str(divide_to_conquer.__name__)}"
    )
